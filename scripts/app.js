...
function validateAndGetForm(form, fields) {
  ...
  const res = {};
  for (const field of fields) {
    const fieldValue = formData.get(field); // получить данные input name="comment"
...
function rerenderHead(activeHabbit) {
  ...
  page.header.progressPercent.innerText = progress.toFixed(0) + '%'; // toFixed(0) - сколько чисел после точки оставить? 0
